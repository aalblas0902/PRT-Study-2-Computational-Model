...
blainstats <- task %>%
  group_by(subject, values.blockcount, values.expgroup, values.responsekeyassignment) %>%
  summarise(
    HIT = sum(hit), 
    FA = sum(fa), 
    MISS = sum(miss), 
  ...
  mutate(
    ...
    log_b_overall = 0.5 * log((total_hits + 0.5) * (total_fa + 0.5) / ((total_miss + 0.5) * (total_cr + 0.5))), 
    log_d_overall = 0.5 * log((total_hits + 0.5) * (total_cr + 0.5) / ((total_miss + 0.5) * (total_fa + 0.5))), 
    total_hits_baseline = HIT_2 + HIT_3, 
    total_cr_baseline = CR_2 + CR_3, 
    total_miss_baseline = MISS_2 + MISS_3, 
    total_fa_baseline = FA_2 + FA_3,
    log_b_baseline = 0.5 * log((total_hits_baseline + 0.5) * (total_fa_baseline + 0.5) / ((total_miss_baseline + 0.5) * (total_cr_baseline + 0.5))), 
    log_d_baseline = 0.5 * log((total_hits_baseline + 0.5) * (total_cr_baseline + 0.5) / ((total_miss_baseline + 0.5) * (total_fa_baseline + 0.5)))
  ) %>%
  ungroup() %>%
  mutate(
    bias_group = if_else(log_b_overall > 0, "Positive Bias", "Negative Bias")
  )


length(unique(blainstats$subject))
# Count how many times each subject appears
table(blainstats$subject)
...
questionnairescored_sdt <- blainstats %>%
  inner_join(questionnaire_scored, by = c("subject" = "subject"))
...
# Summarise results
fit_summary_5 <- do.call(rbind, lapply(names(fit_results), function(sub_id) {
  res <- fit_results[[sub_id]]
  data.frame(
    subject = sub_id,
    rho = res$par[["rho"]],
...
write.csv(fit_summary_5, "fit_summary_5.csv", row.names = FALSE)
